...
def log_likelihood(param_values, k_data, Pk_data):
	...
	matter_electron_pressure_pofk = hmcode_pofk[0, 1, 0]

	matter_pressure_pofk = np.interp(k_data, k, matter_pressure_pofk)
	# Compute the chi^2
	chi2 = np.sum((matter_pressure_pofk - Pk_data)**2/variance)
...
#----------------------------------- 2. Load Data -----------------------------------#
Pk_magneticum, box_size = np.loadtxt('../magneticum-data/data/Pylians/Pk_matterxpressure/Box2_CIC_R1024.txt'), 352
...
fit_params = ['eps_array', 'gamma_array', 'm0_array', 'alpha_array', 'twhim_array']

if __name__=='__main__':
    #----------------------------------- 4. Run emcee -----------------------------------#
    #parallel = 'multiprocessing'
    parallel = 'MPI'
    priors = [[-0.95, 3], [1.05, 3], [10, 17], [0, 1], [6, 7]]
    initial_parameters = [0.2038, 1.33, 13.3, 0.84, 6.65]
    ...
    # Now run long chain
            ...
            np.save('chains/Pk_mp_fit/HMx.npy', walkers)

    fig = corner.corner(flat_chain, labels=fit_params, show_titles=True)
    plt.savefig('chains/Pk_mp_fit/HMx_corner.pdf', dpi=300, bbox_inches='tight')
    ...
    plt.savefig('chains/Pk_mp_fit/HMx_bestfit.pdf', dpi=300, bbox_inches='tight')
